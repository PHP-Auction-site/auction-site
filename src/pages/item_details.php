...
$item_stmt = $pdo->prepare('
    SELECT i.*, c.category_name, u.username AS seller_username 
    FROM items i 
...
$bid_stmt = $pdo->prepare('
    ...
    LIMIT 1
');
$bid_stmt->execute(['item_id' => $item_id]);
$bid = $bid_stmt->fetch(PDO::FETCH_ASSOC);
$highest_bid = $bid['bid_amount'] ?? null;
$highest_bidder = $bid['username'] ?? null;
$has_bid = $bid !== false;
...
$is_active = ($item['status'] === 'Active') && ($end_time > $now);
?>
